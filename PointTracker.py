...
class PointTracker:
    ...
    def createHistogram(self, frame):
        ...
        roi = np.zeros([FRAME_Y, FRAME_X, 3], dtype=hsvFrame.dtype)

        y0, x0 = int(ROW_START * rows), int(COL_START * cols)
        roi = hsvFrame[y0 : y0 + FRAME_Y, x0 : x0 + FRAME_X, :]

        hist = cv2.calcHist([roi], [0, 1], None, [180, L], [0, 180, 0, L])
    ...
    def drawLocker(self, frame):
        ...
        y0, x0 = int(ROW_START * rows), int(COL_START * cols)
        cv2.rectangle(frame, (x0, y0), (x0 + FRAME_X, y0 + FRAME_Y), (0, L - 1, 0), 1)
    ...
    def execute(self, farthestPoint, frame):
        if self.mouseMode:
            ...
            pyautogui.moveTo(
                targetX * self.screenSizeX / frame.shape[1],
                targetY * self.screenSizeY / frame.shape[0],
            )
        else:
    ...
    def histMasking(self, frame, hist):
        ...
        dst = cv2.calcBackProject([hsv], [0, 1], hist, [0, 180, 0, L], 1)
